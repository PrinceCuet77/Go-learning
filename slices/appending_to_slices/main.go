...
func main() {
	...
	fmt.Println(numbers) // Output: [2 3 10]

	// appending more elements at once
	numbers = append(numbers, 20, 30, 40)
	fmt.Println(numbers) // Output: [2 3 10 20 30 40]

	// appending all elements of a slice to another slice
	n := []int{100, 200, 300}
	numbers = append(numbers, n...) // '...' is the ellipsis operator
	fmt.Println(numbers)            // Output: [2 3 10 20 30 40 100 200 300]

	//---------------- Slice's Length and Capacity -----------------------

	nums := []int{1}
	fmt.Printf("Length: %d, Capacity: %d \n", len(nums), cap(nums)) // Output: Length: 1, Capacity: 1

	nums = append(nums, 2)
	fmt.Printf("Length: %d, Capacity: %d \n", len(nums), cap(nums)) // Output: Length: 2, Capacity: 2

	nums = append(nums, 3)
	fmt.Printf("Length: %d, Capacity: %d \n", len(nums), cap(nums)) // Output: Length: 3, Capacity: 4
	...
	fmt.Printf("Length: %d, Capacity: %d \n", len(nums), cap(nums)) // Output: Length: 5, Capacity: 8
	...
	nn := copy(dst, src)
	fmt.Println(src, dst, nn) // Output: [10 20 30] [10 20 30] 3
